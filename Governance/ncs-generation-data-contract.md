...
Use the shallowest route that can solve a material problem.

Limit automated deep rescue to one reentry cycle.

Do not create additional cycles merely to raise an editorial score.

A later final review always scores the exact final draft that will be delivered.

## 8. Final scoring contract

The final delivered draft should have a valid numeric holistic NCS score from the exact final article review.
...
It is not a lifecycle threshold.

Do not require `overall_score >= 8.0` to deliver a completed draft.

Do not require `revision_route = NONE` after the allowed automated revision path has completed.

A 7.x draft can be a legitimate DRAFTED artifact when it is complete and useful for human review.
...
Section-overlap diagnostics are heuristic evidence for the Forensic Editor, not an automatic failure.

If a public-integrity issue is found, give the workflow a focused repair attempt. The purpose is to avoid obviously broken public copy, not to create endless quality loops.
...
The packet `format` field must exactly match:
...
Schema failure is a technical delivery blocker.

## 11. Google Drive delivery

When the complete generated package passes technical completion checks, write the final Google Doc.
...
- a complete generated public article exists
- the exact delivered article has an honest numeric NCS score
- the final social packet exists and is structurally valid
- the article was successfully persisted to Google Drive
- the final fields were successfully written back to the Ideas row

`DRAFTED` does not mean:
...
The lifecycle state records artifact existence. The score records editorial quality. Do not collapse those jobs into one field.

## 14. Failure recovery

If a run fails before a complete artifact is delivered after setting `GENERATING`:

- do not write a fake or partial `draft_url`
- do not write `N/A` into `ncs_score`
- do not replace successful prior final fields
- restore the exact row to `pre_generation_status` when current status is still `GENERATING`
...
Preserve the prior successful `final_title`, `draft_url`, `ncs_score`, `ig_packet_json`, `human_notes`, and `published_url` until a replacement succeeds.
...
Only replace final title, draft URL, score, IG packet, and status after the new delivery is complete.

## 16. Human authority

`DRAFTED` means the automated system delivered the artifact.
