...
Never Coming Soon is a social-first entertainment brand. The artifact is the sensation of discovering a real-feeling movie or show that does not exist. The carousel must be understandable at feed speed and rewarding after a swipe. The image producer owns design and visual execution. The packet owns editorial content.
...
- Locate the single matching row in the Ideas sheet when given a title. Read `ig_packet_json` in full and require `version = ncs_ig_v3`. Title matches that are ambiguous require disambiguation.
- Extract the exact title, format, hook, plot paragraphs, poster tagline, rating, format descriptor, footer copy, art direction, image prompt, and slide/page instructions. Keep a copy of the source text alongside work files for comparison.
...
- The current user's explicit changes outrank the packet. If the packet is blank, malformed, or belongs to an older version, flag it for regeneration rather than translating legacy fields yourself.
...
- Work inside generous safe margins for title, body, rating, and footer. Inspect at full size and at phone-feed size.
...
- Slide 3 belongs primarily to the fictional production. Its palette, medium, type, and central image should change with the film.
- Every slide carries the packet-prescribed quiet footer: **Never Coming Soon** at bottom-left and the exact `0X / 03` page number at bottom-right. Use one fixed baseline and coordinates across the carousel. On the poster, make this footer especially restrained so it does not break the poster illusion.
...
The payoff is a poster somebody could save or share on its own. Develop a singular campaign image and choose whatever medium makes this movie most compelling: graphic, photographic, illustrated, painted, typographic, object-led, or character-led. See [poster direction](poster-direction.md) before starting. Follow exact packet title, tagline, rating, descriptor and footer copy; do not invent real actors or production credits. The poster is allowed and encouraged to feel visually different from the cream editorial slides.

Keep the NCS/page footer present because v3 requires it, but make it visually quiet enough that the poster still reads as the fictional production's artifact.

## Production workflow

1. **Read** the v3 packet, this guide, and the approved example limitations. Identify exact fields and conflicts before designing.
2. **Art direct** the poster conceptually: sketch multiple distinct hero constructions and select the one that communicates the concept and genre with the fewest elements. Ensure Slides 1 and 2 remain the editorial system.
...
4. **Compose** type and repeatable editorial elements deterministically, including packet copy, rating, poster footer, NCS signature, and page numbers. Preserve source art when revising. Keep a layered or reproducible working source if practical.
5. **Check** every line against the packet; image dimensions; paragraph breaks; number/order; text contrast; small-screen legibility; and footer alignment. Inspect each PNG individually, not only a montage.
