...
Do not plan THE CHARACTERS as a row of polished archetype cards. A compressed label may be useful once, but the section should primarily make people exist through behavior, desire, contradiction, relationships, and pressure.
...
Include any known public-copy risks that deserve special care, such as a tempting meta-cut phrase, a coy placeholder where canon already provides a safe concrete noun, or a repeated signature tactic likely to be overspent.
...
When a conflict-driven production contains antagonistic pressure, plan enough cause and effect for the reader to feel that the threat is pursuing something rather than simply delivering escalating tests.

Mystery may protect motive. It should not make tactics feel arbitrary.

When multiple antagonistic actors exist, plan the public article so a first-time reader can understand enough about who is driving pressure, who carries personal history, insider knowledge, or special leverage, and whether recurring actors seem coordinated or distinct when that matters.
...
If canon contains redundant or incoherent threat roles, do not solve canon here. Choose the clearest public emphasis and note the problem in `public_integrity_guard` or another appropriate planning field for the Writer to handle carefully.
...
Prefer a known concrete noun over a coy abstraction when the fact is not a protected reveal. If canon knows the object is an inhaler, custody form, badge number, breaker switch, evidence bag, or seizure medication, plan to name it when doing so increases clarity or pressure.
...
Plan the last public beat as an image, action, sound, line, physical state, or emotional threshold inside the production. Do not plan meta language such as `Cut on...`, `We cut...`, `We stop...`, or another sentence explaining where the article chooses to leave.
...
Do not plan generic review scaffolding such as `The pleasure of the movie is...`, `What makes this work is...`, or `The movie promises...` when the material itself can demonstrate the pleasure.
...
Do not plan public arc-summary sentences such as `tonight she learns`, `by the end he realizes`, `his early mistake later becomes the answer`, or `early on she reaches for a shortcut; later...` when the article can show the behavior and consequence instead.
...
Do not overspend one signature tactic or antagonist beat across the article.

Do not plan character sections as a row of polished functions.
